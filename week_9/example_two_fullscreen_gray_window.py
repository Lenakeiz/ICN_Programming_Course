...
def open_fullscreen_gray_window():
    ...
    try:
        ...
        # Choose which monitor to use (0 = primary, 1 = secondary, etc.)
        screen_index = 1 if len(system_monitors) > 1 else 0
        target_monitor = system_monitors[screen_index]
        print(f"Using monitor index {screen_index}: {target_monitor}")
        ...
        win = visual.Window(
            ...
            screen=screen_index,  # Specify the detected screen number
