from psychopy import visual, event, core, gui, sound, prefs
import os
import numpy as np
...
def demonstrate_mario_game():
    ...
    # Set the default speaker to Realtek audio
    ...
    try:
        ...
        exp_info['images'] = [
            './week_10/assets/Goomba.png', './week_10/assets/Lakitu.png', 
            './week_10/assets/Bill.png', './week_10/assets/SuperHammer.png'
        ]
...
def do_test(win, exp_info):
    ...
    try:
        ...
        for n_trial in range(len(trial_structure)):
            ...
            # Play feedback sound
            sound_file = './week_10/assets/pos_feedback.wav' if trial_data['correct'] else './week_10/assets/neg_feedback.wav'
...
def do_fixation_cross(win, exp_info):
    """Draw a fixation cross using Rect stimuli (avoids Line fillColor bug)."""
    try:
        cross_size = exp_info['fix_cross_dim_pix'] * 2
        line_width = exp_info['fix_line_width_pix']
        ...
        horizontal = visual.Rect(
            ...
            height=line_width,
            pos=(0, 0),
            fillColor=exp_info['palette_white'],
            colorSpace='rgb255',
            lineWidth=0,
            units='pix'
        )
...
def play_feedback(sound_file):
    """Play audio feedback using PsychoPy Sound."""
    try:
        # Load and play the sound file
        feedback_sound = sound.Sound(value=sound_file)
        
        feedback_sound.play()
        core.wait(feedback_sound.getDuration())  # Wait for the sound to finish
        feedback_sound.stop()
...
def save_results(results, participant_id):
    ...
    if not os.path.exists('./week_10/results'):
        os.makedirs('./week_10/results')
    
    # Save to CSV
    df.to_csv(f'./week_10/results/participant_{participant_id}.csv', index=False)
